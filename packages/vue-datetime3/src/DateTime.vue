...
// const isOpen = defineModel("isOpen", { required: false, default: false });
const isOpen = computed<boolean>({
  get() {
    return props.isOpen === undefined ? false : props.isOpen;
  },
...
const inputValue = computed(() => {
  ...
  if (typeof format === "string") {
    return datetime.value
      ? DateTime.fromISO(props.modelValue).setZone(props.zone).toFormat(format)
      : "";
  }
  return datetime.value
    ? datetime.value.setZone(props.zone).toLocaleString(format)
    : "";
});

const popupMinDatetime = computed<DateTime | undefined>(() =>
  props.minDatetime
    ? DateTime.fromISO(props.minDatetime).setZone(props.zone)
    : undefined
);

const popupMaxDatetime = computed<DateTime | undefined>(() =>
  props.maxDatetime
    ? DateTime.fromISO(props.maxDatetime).setZone(props.zone)
    : undefined
);
...
const newPopupDatetime = () => {
  ...
  const roundedMinute =
    Math.round(datetime.minute / props.minuteStep) * props.minuteStep;
...
const popupDate = computed(() =>
  datetime.value ? datetime.value.setZone(props.zone) : newPopupDatetime()
);
