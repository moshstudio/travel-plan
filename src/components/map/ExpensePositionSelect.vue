...
const address = computed<AddressType | undefined>({
  get() {
    if (expense.value) {
      console.log(1, expense.value.location);

      return expense.value.location;
    } else {
      console.log(2, store.expenseSelectAddress);

      return store.expenseSelectAddress;
    }
  },
  set(val) {
    if (!val) {
    ...
    if (expense.value) {
      expense.value.location = val;
      store.updateTravelExpense(toRaw(expense.value));
    } else {
      store.expenseSelectAddress = val;
    }
  },
});
...
const loadData = async () => {
  if (route.params.expenseId) {
    ...
    expense.value = store.getTravelExpenseById(expenseId.value);
  } else {
...
const onSearch = _.debounce(async (value) => {
  ...
  const pointLonlat = center
    ? toLonLat(center)
        .map((i) => i.toFixed(5))
        .join(",")
    : "0,0";

  addressOptions.value = await tdtViewSearch(value, "10000", pointLonlat);
...
<template>
  <div class="relative w-screen h-screen flex flex-col">
    ...
    <div class="relative">
      ...
      >
        <div class="text-xs px-4 py-2 bg-[var(--van-background-2)]">
          以下为当前显示中心位置附近的搜索结果
        </div>
